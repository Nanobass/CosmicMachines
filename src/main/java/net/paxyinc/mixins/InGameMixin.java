...
import net.paxyinc.entities.IRenderableEntity;
...
public abstract class InGameMixin extends GameState {
    ...
    public static void setLocalPlayer(Player player) {
        ...
        if(InGameMixin.player != null) {
            InGameMixin.player.updateCamera(rawWorldCamera, 0.0F);
            UI2.createLocalPlayerUI(InGameMixin.player);
        }
    }
        ...
        for(Map.Entry<UUID, BetterEntity> entry : zi.getEntities().entrySet()) {
            ...
            if(entity instanceof ItemEntity itemEntity) {
                ...
                if(diff.len() < 1 && itemEntity.timer <= 0) {
                    playerInventory.put(itemEntity.items.item, itemEntity.items.amount);
